...
class ChronyManager:
    ...
    def get_gps_offset_ms(self) -> float | None:
        ...
        for s in self._sources:
            if s.name in ("GPS", "PPS", "SHM0", "SHM1"):
    ...
    def generate_config(
        ...
        lines = [
            ...
            "",
            "# GPS time via shared memory (SHM) from gpsd",
            f"# SHM {gps_shm_unit} = coarse GPS time (~10ms accuracy over USB)",
            ...
            "",
            "# PPS timing via shared memory from gpsd",
            f"# SHM {pps_shm_unit} = PPS signal (sub-microsecond when available)",
            f"refclock SHM {pps_shm_unit} refid PPS precision {pps_precision:.0e} "
            f"poll 4 filter 64 trust",
            "",
